...
func TestAccKeycloakOpenIdAudienceResolveProtocolMapper_createAfterManualDestroy(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				PreConfig: func() {
					err := keycloakClient.DeleteOpenIdAudienceResolveProtocolMapper(mapper.RealmId, mapper.ClientId, mapper.ClientScopeId, mapper.Id)
					if err != nil {
						t.Error(err)
					}
				},
...
func getAudienceResolveMapperUsingState(state *terraform.State, resourceName string) (*keycloak.OpenIdAudienceResolveProtocolMapper, error) {
	...
	return keycloakClient.GetOpenIdAudienceResolveProtocolMapper(realm, clientId, clientScopeId, id)
}
